...
from src.ccsort.ccsort import parse_cli, SORT, sort_coordinator, fetch_input
from timeit import Timer


@pytest.mark.parametrize("sys_argv, expected_result", [
    (["ccsort.py", "text.txt"],
     {"input": "text.txt", "output": STD.OUT, "unique": False, "sort": SORT.MERGE, "timer": False}),
    (["ccsort.py", "text.txt", "output.txt"],
     {"input": "text.txt", "output": "output.txt", "unique": False, "sort": SORT.MERGE, "timer": False}),
    (["ccsort.py", "-t", "text.txt"],
     {"input": "text.txt", "output": STD.OUT, "unique": False, "sort": SORT.MERGE, "timer": True}),
    (["ccsort.py", "-u", "text.txt"],
     {"input": "text.txt", "output": STD.OUT, "unique": True, "sort": SORT.MERGE, "timer": False}),
    (["ccsort.py", "-radix-sort", "text.txt"],
     {"input": "text.txt", "output": STD.OUT, "unique": False, "sort": SORT.RADIX, "timer": False}),
    ...
    (["ccsort.py", "-quick-sort", "text.txt"],
     {"input": "text.txt", "output": STD.OUT, "unique": False, "sort": SORT.QUICK, "timer": False}),
    (["ccsort.py", "-sort=quick", "text.txt"],
     {"input": "text.txt", "output": STD.OUT, "unique": False, "sort": SORT.QUICK, "timer": False}),
    (["ccsort.py", "-heap-sort", "text.txt"],
     {"input": "text.txt", "output": STD.OUT, "unique": False, "sort": SORT.HEAP, "timer": False}),
    (["ccsort.py", "-sort=heap", "text.txt"],
     {"input": "text.txt", "output": STD.OUT, "unique": False, "sort": SORT.HEAP, "timer": False}),
    (["ccsort.py", "-heap-sort", "text.txt"],
     {"input": "text.txt", "output": STD.OUT, "unique": False, "sort": SORT.HEAP, "timer": False}),
    (["ccsort.py", "-R", "text.txt"],
     {"input": "text.txt", "output": STD.OUT, "unique": False, "sort": SORT.RANDOM, "timer": False}),
...
@pytest.mark.parametrize("sort_type, pre_sort, post_sort", [
...
# def test_timeit():
#     input = ["B", "A", "E", "D", "C"]
#     t = Timer("""sort_coordinator(input, SORT.RANDOM)""",
#               setup="""from src.ccsort.ccsort import sort_coordinator""")  # t = Timer("""sort_coordinator(input, SORT.MERGE)""")
#     print(t.timeit(10))
